...
Provides consistent exception handling across the system
"""
...
def handle_exception(username, context, exception, show_details=False):
    ...
    # REQUIREMENT: Exception handling - prevent system crashes
...
def safe_execute(func, username, context, *args, **kwargs):
    ...
    try:
        ...
        handle_exception(username, context, e)
        return None
...
def handle_keyboard_interrupt(username, context):
    ...
    log_action(username, f"User interrupted operation in {context}", "Keyboard interrupt (Ctrl+C)", suspicious=False)
